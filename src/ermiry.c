...
static unsigned int ermiry_env_get_mongo_app_name (void) {
...
static unsigned int ermiry_env_get_mongo_db (void) {
...
static unsigned int ermiry_env_get_mongo_uri (void) {
...
static unsigned int ermiry_env_get_private_key (void) {
...
static unsigned int ermiry_env_get_public_key (void) {
...
static void ermiry_env_get_enable_users_routes (void) {
...
static unsigned int ermiry_init_env (void) {
	...
	errors |= ermiry_env_get_mongo_app_name ();

	errors |= ermiry_env_get_mongo_db ();

	errors |= ermiry_env_get_mongo_uri ();

	errors |= ermiry_env_get_private_key ();

	errors |= ermiry_env_get_public_key ();

	ermiry_env_get_enable_users_routes ();

	return errors;

}

static unsigned int ermiry_mongo_connect (void) {
...
static unsigned int ermiry_mongo_init (void) {

	unsigned int retval = 1;

	if (!ermiry_mongo_connect ()) {
		if (!ermiry_roles_init ()) {
			retval = 0;
		}
...
static unsigned int ermiry_init_responses (void) {
	...
	char *status = c_string_create (
		"%s - %s", ERMIRY_VERSION_NAME, ERMIRY_VERSION_DATE
	);
...
unsigned int ermiry_init (void) {
	...
	if (!ermiry_init_env ()) {
		errors |= ermiry_mongo_init ();

		errors |= ermiry_handler_init ();

		errors |= ermiry_users_init ();

		errors |= ermiry_categories_init ();

		errors |= ermiry_places_init ();

		errors |= ermiry_trans_init ();

		errors |= ermiry_init_responses ();
	}
...
static unsigned int ermiry_mongo_end (void) {
...
unsigned int ermiry_end (void) {
	...
	errors |= ermiry_mongo_end ();

	ermiry_roles_end ();

	ermiry_users_end ();

	ermiry_categories_end ();
